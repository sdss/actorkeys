KeysDictionary("alerts", (1,0),
    Key("activeAlerts",
        String(name="alertID", help="alert ID: actor.keyword")*(0,None),
        help="A list of all active alerts.",
    ),
    Key("disabledAlerts",
        String(name="disableInfo", help="(actor.keyword, severity)")*(0,None),
        help="A list of all disabled alerts and their associated severity.",
    ),
    Key("alert",
        String(name="alertID", help="alert ID: actor.keyword"),
        Enum('ok', 'info', 'warn', 'serious', 'critical', name="severity",
            help="The severity level of an alert; ok means the alert is over."),
        String(name="value", help="value of keyword"),
        Bool("disabled", "enabled", name="isEnabled"),
        Bool("noack", "ack", help="has alert been acknowledged?"),
        doCache=False,
    ),
)
